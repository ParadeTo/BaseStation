package com.paradeto.basestation;
...
public class BaseStationDataPreprocess extends Configured implements Tool 
{
	...
	public static class Map extends Mapper<LongWritable, Text, Text, Text> 
	{
		...
		public void map ( LongWritable key, Text value, Context context ) throws IOException, InterruptedException 
		{
			...
			//System.out.println(tableLine.outKey().toString()+"\t"+tableLine.outValue().toString());
	...
	public static class Reduce extends Reducer<Text, Text, NullWritable, Text> 
	{
		...
		{
			this.date = context.getConfiguration().get("date");							//读取日期
			this.rank = Integer.valueOf(context.getConfiguration().get("rank"));		//Read rank
		}
		...
		public void reduce ( Text key, Iterable<Text> values, Context context ) throws IOException, InterruptedException
		{
			...
			try 
			{
				...
				list = SortHashMap.sortHashMap(locs);
